...
#print(os.getcwd())
from codes.experimentclasses.MxyleneAngles import MxyleneAngles
...
diffusion_time = 1.21 #diffusion time controls gaussian kernel radius per gradients paper
...
atoms4 = np.asarray([[6,1,0,7],[6,5,13,15],[15,16,17,13],[7,8,9,0]],dtype = int)



new_MN = False
new_grad = True
savename = 'mxyleneM_013120_cluster'
savefolder = 'mxylene'

loadname = 'mxyleneM_013020_cluster'
loadfolder = 'mxylene'

nreps = 5
if new_MN == True:
    experiment = MxyleneAngles(dim, n, ii, jj,cores,atoms4)
    projector  = np.load(workingdirectory + '/untracked_data/chemistry_data/m-xylene_anglespca_pca50_components.npy')
    experiment.M = experiment.load_data()  # if noise == False then noise parameters are overriden
    experiment.Mpca = RiemannianManifold(np.load(workingdirectory + '/untracked_data/chemistry_data/m-xylene_anglespca_pca50.npy'), dim)
...
src = workingdirectory + '/codes/experiments/mxylene_multirun_tangent_2.py'
